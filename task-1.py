...
# Initialize the graph (adjacency list) with only users and posts
graph = {
    'user1': ['post1', 'post2'],
    'user2': ['post1'],
    'user3': ['post2'],
...
def pagerank(graph, alpha=0.85, max_iter=100):
    ...
    for _ in range(max_iter):
        ...
        for node in graph:
            ...
            new_pr[node] = (1 - alpha) + alpha * sum(pr[neighbor] / len(graph[neighbor]) for neighbor in graph[node] if neighbor in pr)
        pr = new_pr
    return pr
